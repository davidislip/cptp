...
class HiGHSBridge {
 public:
  ...
  void set_separation_interval(int32_t interval) {
    separation_interval_ = interval;
  }

  /// Max cuts to add per separator per round (0 = unlimited).
  /// Jepsen et al. recommend 1: add only the most-violated cut.
  void set_max_cuts_per_separator(int32_t max_cuts) {
    max_cuts_per_sep_ = max_cuts;
  }
  ...
  /// Requires column mapping via undoPrimal to translate between
  /// reduced/original space.
  ...
  /// correction = profit(source) if source==target (tour), 0 if source!=target
  /// (path).
  void set_labeling_bounds(std::vector<double> f, std::vector<double> b,
                           double correction);
  ...
  /// Install domain propagator that fixes edges during B&C based on labeling
  /// bounds.
  ...
  void set_heuristic_node_interval(int64_t interval) {
    heuristic_node_interval_ = interval;
  }
  ...
  void set_fixed_y(std::vector<int8_t> fixed_y) {
    fixed_y_ = std::move(fixed_y);
  }

 private:
  /// Order the visited nodes by following edges from source.
  std::vector<int32_t> order_path(
